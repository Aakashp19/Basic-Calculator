function main() {
  ...
  const templates = {
    calculatorButton: $(`[data-template="calculatorbutton"]`)
      .innerHTML,
  };
  ...
  const mutations = {
    ...
    ADD_PAREN(state, { operator }) {
      const last =
        state.expressions[state.expressions.length - 1] || "";
    ...
    APPEND_OPERAND(state, { value, operator }) {
      ...
      state.currentOperand = newOperand.substring(
        0,
        MAX_NUMBER_LENGTH
      );
    },
    ...
    SHOW_TOTAL(state, { $commit, explicit } = {}) {
      ...
      const isFirstNumber =
        typeof Number(expressions[0]) === "number";
...
const { MathParser, isOperator, operators } = (function (module) {
  ...
  const punctuation = (exports.punctuation = [
    "(",
    ")",
    ...operators,
  ]);
  ...
  const Lexer = (exports.Lexer = class Lexer {
    ...
    lex() {
      const token =
        null || this.getPunctuationToken() || this.getNumericToken();
  ...
  const evaluateAST = (exports.evaluateAST = (node) => {
    ...
    switch (node.type) {
      ...
      default:
        throw new Error(
          `Parsing error: Unrecognized node type "${node.type}"`
        );
    }
  });
